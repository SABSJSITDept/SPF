@include('includes.backend.header')
    ...
    <style>
        ...
        .badge { background: #e65100; color: #fff; font-size: 12px; padding: 3px 10px; border-radius: 12px; margin-left: 10px; }
    ...
    <div>
        <div class="breadcrumb">
            <a href="{{ route('admin.dashboard') }}">Dashboard</a> &rsaquo; Members &rsaquo; Pending
        </div>

        <h1>Pending Members <span class="badge">{{ $registrations->total() }} Records</span></h1>
        ...
        <div style="margin-bottom: 12px; display: flex; gap: 8px;">
            <form method="POST" action="{{ route('admin.members.approveAll') }}" id="approveAllForm" style="display:inline;">
                @csrf
                <button type="button" onclick="confirmSubmit(this.form, 'WARNING: Are you sure you want to approve ALL pending registrations in the database? This cannot be undone quickly.', '#0288d1')"
                    style="background:#0288d1;color:#fff;border:none;padding:8px 16px;border-radius:4px;font-size:13px;font-weight:600;cursor:pointer;">
                    &#10004;&#10004; Approve All
                </button>
            </form>
            <form method="POST" action="{{ route('admin.members.bulkUpdateStatus') }}" id="bulkApproveForm" style="display:inline;">
                @csrf
            ...
            <form method="POST" action="{{ route('admin.members.bulkUpdateStatus') }}" id="bulkRejectForm" style="display:inline;">
                @csrf
        ...
        <div class="table-wrapper">
            @if($registrations->isEmpty())
                <p class="no-records">No pending registrations found.</p>
            @else
            <table id="membersTable">
                <thead>
                    <tr>
                        <th style="width:40px;text-align:center;">
                            <input type="checkbox" id="selectAllCheckbox" style="cursor:pointer;" onclick="toggleAllCheckboxes(this)">
                        </th>
                        <th>#</th>
                ...
                <tbody>
                    ...
                    <tr>
                        <td style="text-align:center;">
                            <input type="checkbox" class="row-checkbox" value="{{ $reg->id }}" style="cursor:pointer;">
                        </td>
                        ...
                        <td>
                            <form method="POST" action="{{ route('admin.members.updateStatus', $reg->id) }}" style="display:inline;">
                                @csrf
                                <input type="hidden" name="status" value="approved">
                                <button type="button" onclick="confirmSubmit(this.form, 'Approve this registration?', '#2e7d32')"
                                    style="background:#2e7d32;color:#fff;border:none;padding:4px 10px;border-radius:4px;font-size:12px;cursor:pointer;">
                                    Approve
                                </button>
                            </form>
                            <form method="POST" action="{{ route('admin.members.updateStatus', $reg->id) }}" style="display:inline;margin-left:4px;">
                                @csrf
                                <input type="hidden" name="status" value="rejected">
                                <button type="button" onclick="confirmSubmit(this.form, 'Reject this registration?', '#c62828')"
                                    style="background:#c62828;color:#fff;border:none;padding:4px 10px;border-radius:4px;font-size:12px;cursor:pointer;">
                                    Reject
                                </button>
                            </form>
