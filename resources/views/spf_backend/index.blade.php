@include('includes.backend.header')
    ...
    <div>
        <div class="breadcrumb">
            <a href="{{ route('admin.dashboard') }}">Dashboard</a> &rsaquo; Members &rsaquo; All Registrations
        </div>

        <h1>All SPF Registrations <span class="badge">{{ $registrations->total() }} Records</span></h1>
                            ...
                            <select name="sub_category" id="filterSubCat">
                                ...
                                @foreach($subCategories as $sub)
                                    <option value="{{ $sub->sub_category_name }}"
                                        ...
                                        {{ request('sub_category') == $sub->sub_category_name ? 'selected' : '' }}>
                    ...
                    <div id="exportFieldsModal" class="modal-overlay" onclick="if(event.target===this)closeExportModal()">
                        <div class="modal-box" style="width:360px;">
                            ...
                            <div class="modal-grid">
                                @php $efIdx = 1; @endphp
                                @foreach(['mid'=>'MID','full_name'=>'Full Name','father_name'=>'Father Name','mobile'=>'Mobile','email'=>'Email','gender'=>'Gender','age'=>'Age','dob'=>'DOB','profession'=>'Profession','prof_category'=>'Prof. Category','state'=>'State','city'=>'City','anchal'=>'Anchal','local_sangh'=>'Local Sangh','sadhumargi'=>'Sadhumargi','working_status'=>'Working Status','referral'=>'Referral','status'=>'Status'] as $fval => $flabel)
                    ...
                    <tr>
                        ...
                        <td>
                            ...
                            <a href="{{ route('admin.members.edit', $reg->id) }}"
                                style="background:#1565c0;color:#fff;border:none;padding:4px 10px;border-radius:4px;font-size:12px;cursor:pointer;text-decoration:none;display:inline-block;">
                                Edit
                            </a>
                            @endif
                        </td>
    ...
    <script>
        ...
        function filterSubCats(reset) {
            var selCatId = document.getElementById('filterProfCat').value;
        ...
        function toggleFilterPanel() {
            document.getElementById('filterHeader').classList.toggle('open');
